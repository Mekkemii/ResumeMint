...
Шаг 1 — Анализ резюме и определение грейда кандидата
- Анализ структуры: цель, опыт, образование, навыки, курсы, «О себе», контакты
- Проверка формата для ATS: файл .docx/.pdf, отсутствие таблиц и картинок, корректные ключевые слова
- Сравнение с HR-стандартами 2022–2025: достижения с цифрами, уровень английского, релевантность опыта, наличие soft skills, отсутствие лишнего

Определение грейда кандидата по критериям:
- Junior (0-2 года опыта): базовое владение технологиями, выполнение типовых задач под контролем лида, ориентация на базовые знания, выполнение задач по плану, проявление инициативы, нуждается в наставничестве
- Middle (2-5 лет опыта): уверенная работа с продуктом, умение оптимизировать решения, сочетание теории и практики, участие в планировании и помощь коллегам, ответственность за отдельные модули, самостоятельное выполнение большинства задач
- Senior (5-8 лет опыта): проектирование архитектуры, умение писать чистый код и решать сложные технические задачи, глубокое понимание архитектурных паттернов и алгоритмов, управление процессами и приоритетами, ведение проекта до релиза и наставничество команды
- Lead/Expert (8+ лет опыта): глубокие технические знания и руководство командами разработчиков, ответственность за успешное выполнение проектов, координация работы команды, принятие стратегических решений, взаимодействие с клиентами и заинтересованными сторонами

Формат оценки грейда: [Грейд] - [краткое обоснование с указанием ключевых факторов: опыт работы, технические навыки, самостоятельность, сложность задач, уровень ответственности]
...
## 3. Анализ по HR-стандартам (2022-2025)
...
## 4. Определение IT-грейда кандидата
- Опыт работы
- Технические навыки
- Самостоятельность
- Сложность задач
...
- Недочеты ⚠️
- Что добавить 🔧
- Определенный грейд кандидата с обоснованием
...
Дай детальный анализ в указанном формате и JSON с оценкой.`;
    ...
    // Возвращаем локальный анализ при ошибке
    return performLocalAnalysis(resumeText);
  }
}
...
function performLocalAnalysis(resumeText) {
  ...
  if (text.includes('8 лет') || text.includes('более 8') || text.includes('expert') || text.includes('руководитель команды') || text.includes('team lead')) {
    level = 'Lead/Expert';
    score += 25;
  } else if (text.includes('5 лет') || text.includes('более 5') || text.includes('senior') || text.includes('архитектор') || text.includes('проектирование')) {
    level = 'Senior';
    score += 20;
...
function generateDetailedAnalysis(resumeText, level, score, skills, strongPoints, weakPoints, recommendations) {
  ...
  analysis += `Оценка:\n`;
  analysis += `[${level}] — ${level === 'Lead/Expert' ? '8+ лет опыта, руководство командами, стратегические решения' : level === 'Senior' ? '5-8 лет опыта, проектирование архитектуры, наставничество' : level === 'Middle' ? '2-5 лет опыта, самостоятельная работа, оптимизация решений' : '0-2 года опыта, базовые знания, наставничество'}.\n\n`;
  ...
  analysis += `Пока итог:\n`;
  analysis += `[${level}] — обоснование: ${level === 'Lead/Expert' ? '8+ лет опыта, руководство командами, стратегические решения' : level === 'Senior' ? '5-8 лет опыта, проектирование архитектуры, наставничество' : level === 'Middle' ? '2-5 лет опыта, самостоятельная работа, оптимизация решений' : '0-2 года опыта, базовые знания, наставничество'}.\n\n`;
